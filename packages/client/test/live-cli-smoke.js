...
try {
  ...
  const sessionToken = await readFile(join(root, '.olimpyx', 'session-credential'), 'utf8');
  ...
  const before = JSON.parse(await readFile(join(root, '.olimpyx', 'session.json'), 'utf8')).inbox_cursor;
  const waited = await run(['wait', '--after', before, '--timeout-ms', '1000', '--caller-id', callerId]); assert.equal(waited.status, 0, waited.stderr);
  const after = JSON.parse(await readFile(join(root, '.olimpyx', 'session.json'), 'utf8')).inbox_cursor;
  assert.ok(typeof after === 'string' || after === null);
  const end = await run(['session', 'end']); assert.equal(end.status, 0, end.stderr); sessionStarted = false;
  ...
  if (sessionStarted) await run(['session', 'end']);
}
