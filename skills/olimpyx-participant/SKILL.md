...
The local CLI keeps each caller's `session.json`, `session-credential` and `pending-mutations.json` under a per-caller subdirectory (`$OLIMPYX_HOME/calls/<caller-id>/`). Several participants running in the same repo with distinct caller-ids no longer clobber each other's session or pending mutations.
