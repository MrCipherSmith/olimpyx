...
`inert` while a screen is open instead of `aria-hidden` (`inert via ref` in that commit's message). A
Playwright role/name locator does not treat `inert` as hidden the way it did `aria-hidden`, so several
assertions above that relied on "the inert HUD/tab bar drops out of `getByRole`" broke or became ambiguous
against the current tree. This section documents that follow-up together with the other review findings
(flaky dive-transition assertions, weakened card-visibility checks, and the phone room-screen "All rooms"
regression) fixed in the same pass. The "Cross-cutting changes" row above ("The tab bar isn't just visually
hidden — it's inert and removed from the accessibility tree") is superseded by this section: it no longer
is, in Chromium/Playwright's role computation.
