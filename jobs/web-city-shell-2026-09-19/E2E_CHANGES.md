...
## Review pass (2026-09-20): flakiness, weakened intent, and the `inert` follow-up

A later commit (`edfe982`, after this rewrite landed) changed `CityShell.tsx` to make the HUD/city
`inert` while a screen is open instead of `aria-hidden` (`inert via ref` in that commit's message).
Chromium's own accessibility tree does drop an `inert` subtree, same as `aria-hidden` — but Playwright's
`getByRole`/`getByName` locators compute roles and names from the DOM/ARIA tree directly and do not
consult `inert` (or the live accessibility tree) at all, so a role/name locator still matches inside an
inert subtree even though a real screen reader would not see it. That gap is what broke or made ambiguous
several assertions above that relied on "the inert HUD/tab bar drops out of `getByRole`". This section
documents that follow-up together with the other review findings (flaky dive-transition assertions,
weakened card-visibility checks, and the phone room-screen "All rooms" regression) fixed in the same pass.
The "Cross-cutting changes" row above ("The tab bar isn't just visually hidden — it's inert and removed
from the accessibility tree") is superseded by this section: that's true of Chromium's real accessibility
tree, but not of Playwright's own role computation, which is what `getByRole` actually queries.

| File / line(s) | Old assertion | New assertion | Reason |
|---|---|---|---|
| `city-shell-nav.spec.ts`, deep-link test | `.city-shell-world` `toHaveAttribute('aria-hidden', 'true')` | `toHaveAttribute('inert', '')` | `CityShell.tsx` now sets `inert` via a ref, not `aria-hidden`. |
| `showcase.spec.ts`, "HUD navigation opens each screen…" | `page.getByRole('button', { name: /Refresh/ }).click()` | `page.locator('.screen-layer').getByRole('button', { name: /Refresh/ }).click()` | The room screen's Refresh action and the HUD account's copy are the same React element reference (`refresh`/`refreshButton` in `App.tsx`/`PublicShowcase.tsx`), rendered in two places at once; the HUD's copy is inert while the screen is open, but a role query still matches it (strict-mode violation) since `inert` isn't treated as hidden. Same fix applied in `showcase-edge-cases.spec.ts` ("authenticated read 401…") and `observatory.spec.ts` (scoped the "Browser room …" button lookups to `.room-list` instead, same root cause against the city's own accessible building list). |
| `showcase.spec.ts`, "mobile room prioritizes history…" | `getByRole('navigation')).toHaveCount(0)` | `expect(page.locator('.city-shell-hud')).toHaveAttribute('inert', '')` | The phone tab bar is a `<nav>` inside the (inert) HUD; a role query still finds it (see above), so assert the actual `inert` attribute on its ancestor instead of an accessible-tree count. |
| `city-shell-nav.spec.ts`, "opening the Library…" (was L39–49) and "Escape cancels…" (was L88–98) | Assertions timed against real wall-clock (`toHaveClass(/visible/, { timeout: 2000 })` racing the 650ms/550ms focus/dive phases) and a fixed `page.waitForTimeout(1300)` to prove a cancelled dive never lands | `page.clock.install()` + `page.clock.runFor(650)` / `runFor(550)` / `runFor(1300)`, asserting `.dive-overlay`'s `data-phase` attribute at each step | `DIVE_TIMINGS` in `diveMachine.ts` are short enough (650ms/550ms) that asserting mid-transition against real time is racy on a loaded CI box, and the fixed 1300ms wait only proved the (fast) machine had already finished by then, not that it deterministically never fires a stale timer. A virtual clock makes each phase transition an explicit, instant step. (An earlier version of this row claimed `DiveOverlay.tsx`'s visible "packet dive" label — set from a `useEffect` — never flushes under `page.clock` and dropped the overlay-label assertion in favour of the `.dive-status` live region alone. That claim was wrong: `page.clock` (via `@sinonjs/fake-timers`) only fakes `setTimeout`/`setInterval`/`requestAnimationFrame`/`Date`, not the microtask queue or `MessageChannel` that React's Scheduler uses to flush passive effects, so the `useEffect` runs normally on the real event loop regardless of virtual time. Re-verified across several repeated runs: the "opening the Library" test now asserts both the decorative `.dive-overlay-name` text and the effect-free `.dive-status` live region.) |
| `city-shell-nav.spec.ts` | — | New test: "opening a room from the Rooms directory redives into it, with motion enabled" | Coverage gap: the existing Rooms→room "redive" test (`opening a room from the directory dives straight into it…`) only ran under `prefers-reduced-motion: reduce`; the new test exercises the same `redive` branch in `diveMachine.ts` with motion enabled (focus → dive → overlay → landing, not an instant open). |
| `showcase.spec.ts`, "the city overview is reachable…" | Only checked the published room appears as a building | Also opens Knowledge and asserts the published card's topic text is visible, then returns to the city | Re-adds a published-knowledge-card visibility check (dropped when the old dashboard's `getByText(card.latest.topic).first()` on `/` was replaced — the overview's accessible building directory only lists buildings, not individual cards, so the check now goes through the Knowledge screen instead). |
| `showcase.spec.ts`, "all public views…" (`/?view=knowledge&card=…`) and `participant-layout.spec.ts` (same route) | Only asserted the shared "Central Library of Knowledge" screen heading | Also asserts the card's own `<h2>` topic heading (and, in `showcase.spec.ts`, its summary text scoped to `.card-detail`) | The shared screen heading is identical for every card and doesn't prove the *specific* card rendered; `KnowledgePanel.tsx`/`PublicKnowledge.tsx` render the topic as its own heading in the detail panel. |
| `shell.css` `.screen-room .screen-actions a` (phone width) + `participant-layout.spec.ts`, `showcase.spec.ts` ("mobile room…") | "All rooms" `display: none`; tests asserted `toBeHidden()` | CSS: collapses to a 38×38px icon-only compact button (`#`, `text-indent` off-screen + a `::before` glyph with `content: '#' / ''` so the glyph doesn't also join the accessible name) instead of disappearing. Tests: assert `toBeVisible()`, click it, and confirm it opens the room list, in addition to the existing "Back to the city" + tab-bar round trip | Review finding: the tab bar the old comment called "reachable" is actually inert/covered while a screen is open, so phones had no compact way back to the room list other than a full close-and-reopen. Verified this still clears the 55%-of-viewport history contract at 320×568 (`>57%` measured, vs. the `>55%` requirement). |
